    ...
    const fetchUploads = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/alluploads"
        );
          ...
          {uploads.map((upload) => (
            <li key={upload._id} className="allpost-item">
              <div className="allpost-item-left">
                <Link
                  to={`/getprofile/${upload.rollNo}`}
                  className="allpost-profile-link"
                >
                  <p className="allpost-name">{upload.name}</p>
                </Link>
                <p>
                  {upload.description.length > MAX_LENGTH
                    ? expandedItems[upload._id]
                      ? upload.description
                      : upload.description.substring(0, MAX_LENGTH) + "..."
                    : upload.description}
                </p>
                {upload.description.length > MAX_LENGTH && (
